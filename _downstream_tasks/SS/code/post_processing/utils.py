...
def prob_to_secondary_structure(ensemble_outputs, label_mask, seq, name, args, base_path):
    ...
    if args.plots:
        ...
        VARNA = os.path.join(base_path, 'utils/VARNAv3-93.jar')
        ...
        sp1_std=subprocess.Popen(["java", "-cp", VARNA, "fr.orsay.lri.varna.applications.VARNAcmd", '-i', plot_input, '-o', rad_output, '-algorithm', 'radiate', '-resolution', '8.0', '-bpStyle', 'lw', '-auxBPs', tertiary_bp], stderr=subprocess.STDOUT, stdout=subprocess.PIPE).communicate()[0]
        print(sp1_std.decode())
        sp2_std=subprocess.Popen(["java", "-cp", VARNA, "fr.orsay.lri.varna.applications.VARNAcmd", '-i', plot_input, '-o', lin_output, '-algorithm', 'line', '-resolution', '8.0', '-bpStyle', 'lw', '-auxBPs', tertiary_bp], stderr=subprocess.STDOUT, stdout=subprocess.PIPE).communicate()[0]
